...
--CREATE LOGIN
--##########################################################
IF NOT EXISTS (SELECT name FROM sys.server_principals WHERE name = 'sacs_login')
	BEGIN
		CREATE LOGIN sacs_login WITH PASSWORD = 'P@ss00word!'
		print 'sacs_login - login has been created'
	END
GO  
...
--CREATE USER
--##########################################################
IF NOT EXISTS (SELECT name FROM sys.database_principals WHERE name = 'sacs_login_user')
	BEGIN
		CREATE USER sacs_login_user FOR LOGIN sacs_login
		print 'sacs_login_user - user has been created'
	END		
GO  

sp_addrolemember 'db_owner',  'sacs_login_user'
print 'sacs_login_user added role as db_owner'
