...
--CREATE LOGIN
--##########################################################
IF NOT EXISTS (SELECT name FROM sys.server_principals WHERE name = 'smpcs_login')
	BEGIN
		CREATE LOGIN smpcs_login WITH PASSWORD = 'P@ss00word!'
		print 'smpcs_login - login has been created'
	END
GO  
...
--CREATE USER
--##########################################################
IF NOT EXISTS (SELECT name FROM sys.database_principals WHERE name = 'smpcs_login_user')
	BEGIN
		CREATE USER smpcs_login_user FOR LOGIN smpcs_login
		print 'smpcs_login_user - user has been created'
	END		
GO  

sp_addrolemember 'db_owner',  'smpcs_login_user'
print 'smpcs_login_user added role as db_owner'
